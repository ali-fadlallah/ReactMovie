...
export default function People() {
  ...
  async function searchWords(eventInfo) {
    ...
    if (eventInfo.target.value == "") {
      ...
      let { data } = await axios.get(`https://api.themoviedb.org/3/search/person?api_key=${apiKey}&language=en-US&query=${eventInfo.target.value}&page=1&include_adult=false`);
  ...
  async function getData(category, pageNo) {
    ...
    let { data } = await axios.get(`https://api.themoviedb.org/3/person/${category}?api_key=${apiKey}&language=en-US&page=${pageNo}`);
        ...
        <>
          ...
          {responseAPI != null ?
            ...
            <div>
              ...
              <input onChange={searchWords} className="mt-5 form-control w-50 m-auto bg-transparent text-white text-center" type="search" placeholder="Search..." />
              ...
              <div className='row g-3 my-4 text-center'>
                ...
                {responseAPI.map((element, index) =>
                  ...
                  <div key={index} className=' col-md-2'>
                    <Link to={"/details/" + element.id + "/people"}>
                      <div className=' position-relative'>

                        {element.profile_path == null ? <img src={noImage} className="w-100" alt="no_image" /> : <img className=' rounded w-100' src={"https://image.tmdb.org/t/p/w500" + element.profile_path} alt="posterImage" />}
