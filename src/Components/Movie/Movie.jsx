...
export default function Movie() {
  ...
  let searchWords = async (eventInfo) => {
    ...
    if (eventInfo.target.value == "") {
      ...
      let { data } = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&language=en-US&query=${eventInfo.target.value}&page=1&include_adult=false`);
              ...
              <div className='row g-3 my-4 text-center'>
                ...
                {responseAPI.map((element, index) =>
                  ...
                  <div key={index} className=' col-md-2'>
                    <Link to={"/details/" + element.id + "/movie"}>
